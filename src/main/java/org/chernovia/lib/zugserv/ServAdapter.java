...
public abstract class ServAdapter implements ZugServ {
	...
	private final Logger logger = Logger.getLogger("ServLog");
	...
	public Vector<Connection> getAllConnections(boolean active) {
		if (active) {
			Vector<Connection> loggers = new Vector<>();
	...
	public void newMsg(Connection conn, int chan, String msg) {
		...
		if (msg.equalsIgnoreCase("EXIT")) {
			...
			StringBuilder list = new StringBuilder("Currently connected: " + newline);
			for (Connection c: conns) list.append(c.getHandle()).append(newline);
			conn.tell("who", list.toString());
		}
		else handled = false;
